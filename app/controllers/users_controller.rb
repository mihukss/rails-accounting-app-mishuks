class UsersController < ApplicationController
  ...
  def create
    # pp permitted_params["actor"][:first_name]
    # first_name = permitted_params.delete("actor")
    pp permitted_params
    user = User.new permitted_params
    # user.build_actor(first_name: first_name[:first_name])
    user.save!
    if user.save
  ...
  def permitted_params
    params.require(:user).permit(:email, :password, {actor: :first_name}, :id)
  end
end